...
module.exports = async (req,res) => { const t=await requireAuth(req,res); if(!t)return; const {repo,id,file}=req.query; if(!/^[^/]+\/[^/]+$/.test(repo||'') || !/^\d+$/.test(String(id||''))) return json(res,400,{error:'invalid_request'}); try {
  ...
  if(file){ const e=entries.find(x=>x.entryName===file); if(!e)return json(res,404,{error:'file_not_found'}); res.setHeader('Content-Type',mime(e.entryName)); res.setHeader('Cache-Control','private, max-age=300'); return res.status(200).send(e.getData()); }
