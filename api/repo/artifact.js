...
module.exports = async (req,res) => { const t=requireAuth(req,res); if(!t)return; const {repo,id,file}=req.query; if(!/^[^/]+\/[^/]+$/.test(repo||'') || !/^\d+$/.test(String(id||''))) return json(res,400,{error:'invalid_request'}); try {
